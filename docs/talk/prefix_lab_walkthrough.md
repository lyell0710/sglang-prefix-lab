# 面试讲稿 · SGLang 前缀缓存实验室(P01-P06 成稿)
...
- **调度**:lpm 轻载与 fcfs 无可区分(差值与轮间波动同量级——按预锁阈值如实判平,
  这本身是方法论卖点);192-req 积压档 lpm p99 反劣 13%、命中 −2.4pp(>2σ)。
  机理候选:>128 窗口退化 + in-batch 去重 deprioritize;已标注为推断未剖验。
...
被证伪当场改理论笔记(EXP-P02)。工程细节:uv venv 符号链接的进程身份校验、
