...
```mermaid
flowchart LR
    A["token 契约<br/>EXP-P01 / P02"] --> B["命中收益<br/>EXP-P03 / P07"]
    B --> C["调度策略<br/>EXP-P04 / P08"]
    B --> D["逐出边界<br/>EXP-P05"]
    B --> E["多副本路由<br/>EXP-P06"]
```
...
**3. LRU 逐出不是斜坡,是悬崖。** 热前缀命中率只有 1.0 与约 0.06 两个稳态:池容量 ≥ 重用距离 D=8192×(1+cold_ratio) 时全命中,越线即崩,三种池容量 × 四档冷流量无一中间态。工程含义:容量规划应按重用距离而非热集大小估算;冷流量插队会线性推远热前缀的重用距离,使原本充足的池越过该边界。
    ...
    async with session.post(url + "/v1/chat/completions", json=payload) as r:
        async for raw in r.content:
            ...
            if u and u.get("prompt_tokens_details"):
                cached = u["prompt_tokens_details"].get("cached_tokens")  # server 报告的命中 token 数
