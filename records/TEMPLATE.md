...
- **对数值统一方法**：同 SEED、同输入；自研引擎逐层 dump 中间 tensor，与 HF
  `output_hidden_states=True` / 各层 forward hook 输出对齐；记 **max_abs_err + mean_abs_err**。
...
- **大 dump 铁律**：入库「首末层 + 抽样中间层 + 最终 logits top-k」；全量本体放 data/local/
  （gitignore），仓里入 manifest（路径 + sha256 + provenance 行）。
...
## 2. 环境与配置
模型 + 尺寸 + dtype + seq_len + SEED + commit；完整命令与环境变量；硬件占用（哪张卡）；
共享状态（权重缓存冷热、是否复用进程）。
...
## 4. 原始数据
文件指针列表（data/raw/EXP-Dxx/...，每个文件首行 provenance）；
任何只来自终端输出而无 raw 的数字**必须在此注明**「终端级证据」。
